...
namespace Memoria.Persona5T.BeepInEx;

public static class ExtensionMethods
{
public static IReadOnlyList<T> DistinctBy2<T, TKey>(this IEnumerable<T> self, Func<T, TKey> selector)
    {
        List<T> result;
        ...
        Func<TSourceValue, TTargetValue> valueSelector)
    {
        ...
        if (il2cpp._comparer.Pointer != Il2CppSystem.Collections.Generic.EqualityComparer<TSourceKey>.Default.Pointer)
            throw new ArgumentException($"The IL2CPP Dictionary uses a non-standard Comparer ([{il2cpp._comparer}]) that cannot be converted to a Managed type.", nameof(il2cpp));
